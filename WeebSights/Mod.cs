...
[Injectable(TypePriority = OnLoadOrder.PostDBModLoader + 65535)]
public class Mod(
    ...
    WeebLootService weebLootService
) : IOnLoad
{
    public Task OnLoad()
    {
        ...
        if (
            !weebLocaleService.TryLoadLocales(
                Path.Join(currentAssemblyPath, "/db/locales/en.json"),
                out var locales
            )
        )
        ...
        if (
            !weebLootService.TryLoadConfig(
                Path.Join(currentAssemblyPath, "/db/loot/default.jsonc"),
                out var lootPreset
            )
        )
            logger.Critical(
                "[Weeb Iron Sights] Failed to load presets. Loot may not spawn in raid"
            );

        if (
            !weebItemService.TryLoadConfig(
                Path.Join(currentAssemblyPath, "/db/items.jsonc"),
                out var items
            )
        )
        {
        ...
        foreach (var item in items)
            weebLootService.RegisterToStaticLoot(lootPreset, item.Id);
